...
class JSP(Database):
    def __init__(self, data):
        ...
        self.maximum_op = np.array([len(self.ct[qad_code]) for qad_code in self.job.qad_code])
        ...
        self.job_id, self.machine_name = None, None
        self.logs = []
    ...
    def reward(self, job_id):
        ...
        reward = (r[job_id] - self.r[job_id]) / np.sum(self.maximum_op) * 100
        self.r = r
        return reward

    def step(self, action):
        condition = (self.current_op < self.maximum_op) & (self.start_time <= self.time_step)
        if action == 0:
        ...
        if done:
            self.time_step = np.max(self.start_time) + 0.001
            r = self.reward(job_id)
            self.state = self.get_state()
            return self.state, r, done
        else:
            trial = 0
            condition = (self.current_op < self.maximum_op) & (self.start_time <= self.time_step)
            while np.sum(condition) == 0:
                self.time_step = np.sort(np.unique(self.start_time))[trial]
                trial += 1
                condition = (self.current_op < self.maximum_op) & (self.start_time <= self.time_step)
            self.update_doing()
